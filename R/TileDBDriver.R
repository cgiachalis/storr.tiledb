...
#' back-end.
#'
#' This class is intended for usage in [storr] or [TileDBStorr].
#'
...
TileDBDriver <- R6::R6Class(
  ...
  public = list(
    ...
    get_keymeta_unit = function(key, namespace, meta_col = c("expires_at", "notes")) {
      ...
      sp <- list(namespace = namespace, key = key)
      val <- arrobj$tiledb_array(extended = FALSE,
                                 attrs = meta_col,
                                 selected_points = sp,
                                 return_as = "arrow")[]
      ...
      # TODO: Remove when TileDB fixes i
    ...
    export_tdb = function(key,
      ...
      idx <- self$filter_keys(key, namespace)[]
      ...
      if (self$serial_format != dest_driver$serial_format) {
        ...
        dsto <- TileDBStorr$new(dest_driver, "objects")
        dsto$mset(idx$key, idx$namespace,
                  value = values,
                  expires_at = idx$expires_at,
                  notes = idx$notes, use_cache = FALSE)
