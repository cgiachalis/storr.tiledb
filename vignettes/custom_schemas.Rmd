...
date: "2026-09-05"
vignette: >
  %\VignetteIndexEntry{Schema Configuration}
...
To tune TileDB storr performance and storage characteristics, `storr.tiledb` provides
R6 classes that abstracts the complexities when comes to schema configuration. 
...
What can be customised: compression algorithms, compression levels, tile capacity,
cell order, and tile order settings for both the keys and data arrays.
...
This is useful only if you wish to apply selectively compression to specific
attributes and/or dimensions. Otherwise, prefer `compression_level = NULL`
in `storr_tiledb()`.
